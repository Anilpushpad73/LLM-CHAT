...
router.put('/:chatId/title', authenticate, async (req, res) => {
    try {
        const { chatId } = req.params;
        ...
        const updatedChat = await Chat.findByIdAndUpdate(chatId, { title }, { new: true });
    ...
    try {
        ...
        try {
            ...
            if (!geminiApiKey) {
                ...
                const response = await axios.post(`${GEMINI_API_URL}?key=${geminiApiKey}`, {
