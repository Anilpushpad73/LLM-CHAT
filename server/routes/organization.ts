...
router.post('/create', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (user) {
      user.activeOrganizationId = organization._id as Types.ObjectId;
      await user.save();
    }
...
router.put('/:orgId/rename', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const { orgId } = req.params;
    const { name } = req.body;
...
router.post('/:orgId/invite', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const { orgId } = req.params;
...
router.put('/switch/:orgId', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const { orgId } = req.params;
